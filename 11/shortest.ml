...
let shortest_tour m n saida chegada =
	let rec resolver_cadea l x = (* Convierte la lista de puntos en una lista de tuplas *)
		match x with
			  Raiz -> l
	...
	in
	let rec buscar_sol = function (* Comprueba si algún punto de la lista es solución *)
		  [] -> None
		| h::t ->
			match h with
				  Raiz -> None
				| Punto (x, y, _) ->
					...
					then
						...
						buscar_sol t
	in
	let rec anadir_puntos_pasados pp = function
		  [] -> pp
		| h::t ->
			match h with
				  Raiz -> pp
				| Punto (x,y,_) -> anadir_puntos_pasados ( (x,y)::pp ) t
	in
	...
	let rec buscar puntos_pasados posibilidades =
		if posibilidades = [] then
			...
			match buscar_sol posibilidades with
				  None -> buscar (anadir_puntos_pasados puntos_pasados posibilidades) (lista_casillas puntos_pasados [] posibilidades)
